...
@compute @workgroup_size(8, 8) fn cs(
    ...
    var ambientColor = vec4<f32>(0.0, 0.0, 0.0, 1.0);
    ...
    for (var g = 0; g < numRays; g++) {
      for (var i = 0; i < numTris; i++) {
          let currTri = createTri(i * 4);
          let intersectionResult = rayIntersect(rayOrigin + rand(camera.w * f32(g))*rand(camera.w / f32(g + 1))*vec3f(vec2f(rand(camera.w/10.0 + f32(g)) * 2.0 - 1.0, rand(camera.w/10.0 - f32(g)) * 2.0 - 1.0), 0.0), rayDir, currTri);
      ...
      if (foundIntersection) {
        ...
        ambientColor += vertColorBuffer[index * 4 + 3] * 40.0/f32(numRays);
        // make new ray
        ...
        if (dot(newHemiRay, closestNormal) < 0.0) {
          ...
          // basic shadow
          // find intersection
          var closestAmbient = vec4f(9000.0);
          var ambientIndex = i32(0);
          for (var j = 0; j < numTris; j++) {
          
            let tri = createTri(j * 4);
            let shadowRay = normalize(lightPosition - closestIntersection.xyz);
            let shadowResult = rayIntersect(closestIntersection.xyz + shadowRay*vec3(0.01), shadowRay, tri);
            
            let triResult = rayIntersect(closestIntersection.xyz + newHemiRay*vec3(0.01), newHemiRay, tri);
            if (triResult.w != 0) {
              if(squaredDistance(closestIntersection.xyz, triResult.xyz) < squaredDistance(closestAmbient.xyz, triResult.xyz)) {
                closestAmbient = triResult;
                ambientIndex = j;
              }
            }
          ...
          if (closestAmbient.w < 8999.0) {
            ambientColor += vertColorBuffer[ambientIndex * 4 + 3]; 
          }
    ...
    if (hitSphere == -1.0) {
        if (foundIntersection) {
            //textureStore(tex, pos, tempColor/f32(numRays));
            textureStore(tex, pos, tempColor/f32(numRays) + ambientColor/f32(numRays*4));
            //textureStore(tex, pos, ambientColor/f32(numRays*4));
  ...
  function render() {
    ...
    pass.dispatchWorkgroups(texture.width/8, texture.height/8); // could be optimized
    pass.end();
